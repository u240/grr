...
class Distribution(rdf_structs.RDFProtoStruct):
  ...
  def Record(self, value):
    """Records given value."""
    self.sum += value  # pyrefly: ignore[missing-attribute]
    self.count += 1  # pyrefly: ignore[missing-attribute]
...
class MetricMetadata(rdf_structs.RDFProtoStruct):
  ...
  def DefaultValue(self):
    if self.value_type == self.ValueType.INT:  # pyrefly: ignore[missing-attribute]
      return 0
    elif self.value_type == self.ValueType.FLOAT:  # pyrefly: ignore[missing-attribute]
      return 0.0
    elif self.value_type == self.ValueType.DISTRIBUTION:  # pyrefly: ignore[missing-attribute]
      return Distribution()
    else:
