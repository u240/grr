...
"""
    ...
    args = self.grr_api.types.CreateFlowArgs(flow_name="YaraProcessScan")  # pyrefly: ignore[missing-attribute]
...
"""
    ...
    args = self.grr_api.types.CreateFlowArgs(flow_name="YaraProcessScan")  # pyrefly: ignore[missing-attribute]
    args.yara_signature_blob_id = self.grr_api.UploadYaraSignature(signature)  # pyrefly: ignore[missing-attribute]
    args.process_regex = _GetProcessNameRegex(self.client)
    args.ignore_grr_process = False
...
class TestProcessDump(test_base.AbstractFileTransferTest):
  ...
  def runTest(self):
    args = self.grr_api.types.CreateFlowArgs(flow_name="DumpProcessMemory")  # pyrefly: ignore[missing-attribute]
