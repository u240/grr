...
def main(argv):
  ...
  command_signer_args = [
      "--private-key",
      command_signing_key_file.name,
      "--api_endpoint",
      f"http://localhost:{api_port}",
      # pylint: disable=line-too-long
      # pyformat: disable
      "--template-param",
      f"E2E_TEST_EXECUTABLES={ROOT_DIR / 'test' / 'grr_response_test' / 'test_data'}",
      str(ROOT_DIR / "config" / "command_execution" / "e2e_test.textproto"),
      str(ROOT_DIR / "config" / "command_execution" / "flow_and_artifact.textproto"),
  ...
  if _OSQUERY_PATH.value:
    command_signer_args.extend([
        ...
        # pylint: disable=line-too-long
        # pyformat: disable
        str(ROOT_DIR / "config" / "command_execution" / "osquery.textproto"),
        # pylint: enable=line-too-long
        # pyformat: enable
    ])
