...
class HuntTest(
  ...
  def testStopHuntIfCrashLimitExceededDoesNothingIfHuntIsNotRunning(self):
    hunt_obj = models_hunts.CreateDefaultHunt()
    hunt_obj.creator = self.test_username
    hunt_obj.crash_limit = 1
    data_store.REL_DB.WriteHuntObject(hunt_obj)
    ...
    # The crash limit is exceeded but the hunt is not running anymore.
    client_id = db_test_utils.InitializeClient(data_store.REL_DB)
    db_test_utils.InitializeFlow(
        data_store.REL_DB,
        client_id,
        flow_id=hunt_obj.hunt_id,
        parent_hunt_id=hunt_obj.hunt_id,
        flow_state=flows_pb2.Flow.FlowState.CRASHED,
    )

    hunt.StopHuntIfCrashLimitExceeded(hunt_obj.hunt_id)
  ...
  def testStopHuntIfCPUOrNetworkLimitsExceededDoesNothingIfHuntIsNotRunning(
      self,
  ):
    hunt_obj = models_hunts.CreateDefaultHunt()
    hunt_obj.creator = self.test_username
    hunt_obj.total_network_bytes_limit = 1000
    data_store.REL_DB.WriteHuntObject(hunt_obj)
    ...
    # The total network bytes limit is exceeded but the hunt is not running
    # anymore.
    client_id = db_test_utils.InitializeClient(data_store.REL_DB)
    db_test_utils.InitializeFlow(
        data_store.REL_DB,
    ...
    hunt.StopHuntIfCPUOrNetworkLimitsExceeded(hunt_obj.hunt_id)

    hunt_obj = data_store.REL_DB.ReadHuntObject(hunt_obj.hunt_id)
    self.assertEqual(hunt_obj.hunt_state, hunts_pb2.Hunt.HuntState.COMPLETED)

  def testStopHuntIfCPUOrNetworkLimitsExceededStopsHuntOnlyOnce(self):
    hunt_obj = models_hunts.CreateDefaultHunt()
    hunt_obj.creator = self.test_username
    hunt_obj.total_network_bytes_limit = 1000
    hunt_obj.avg_network_bytes_per_client_limit = 100
    data_store.REL_DB.WriteHuntObject(hunt_obj)
    ...
    # Both the total network bytes limit and the average network bytes per
    # client limit are exceeded: the hunt should be stopped because of the
    # former and the latter should not attempt to stop it again.
    with mock.patch.object(hunt, "MIN_CLIENTS_FOR_AVERAGE_THRESHOLDS", 1):
      hunt.StopHuntIfCPUOrNetworkLimitsExceeded(hunt_obj.hunt_id)
    ...
    self.assertEqual(
        ...
        hunts_pb2.Hunt.HuntStateReason.TOTAL_NETWORK_EXCEEDED,
    )
  ...
  def testHuntClientRateIsAppliedCorrectly(self):
    ...
    # already.
    self.assertEmpty(data_store.REL_DB.ReadFlowRequests(client_ids[0], hunt_id))

    for i, client_id in enumerate(client_ids[1:]):
      flow_obj = data_store.REL_DB.ReadFlowObject(client_id, hunt_id)

      flow_requests = data_store.REL_DB.ReadFlowRequests(client_id, hunt_id)
      self.assertLen(flow_requests, 1)
      flow_request, _ = flow_requests[flow_obj.next_request_to_process]

      time_diff = rdfvalue.RDFDatetime(flow_request.start_time) - (
