...
class ArtifactTest(flow_test_lib.FlowTestsBaseclass):
  ...
  def RunCollectorAndGetResults(
    ...
    return flow_test_lib.GetUnpackedFlowResultsOfTypes(
        client_id,  # pyrefly: ignore[bad-argument-type]
        session_id,
...
class GrrKbWindowsTest(GrrKbTest):
  ...
  def setUp(self):
    ...
    os_overrider = vfs_test_lib.VFSOverrider(
        rdf_paths.PathSpec.PathType.OS, vfs_test_lib.FakeFullVFSHandler  # pyrefly: ignore[missing-attribute]
    )
    os_overrider.Start()
    self.addCleanup(os_overrider.Stop)

    reg_overrider = vfs_test_lib.VFSOverrider(
        rdf_paths.PathSpec.PathType.REGISTRY,  # pyrefly: ignore[missing-attribute]
...
class GrrKbDarwinTest(GrrKbTest):
  ...
  def testKnowledgeBaseRetrievalDarwin(self):
    ...
    with vfs_test_lib.VFSOverrider(
        rdf_paths.PathSpec.PathType.OS,  # pyrefly: ignore[missing-attribute]
  ...
  def testWindowsListUsersDir(self):
    ...
    class ActionMock(action_mocks.ActionMock):
      ...
      def GetFileStat(
        ...
        args = mig_client_action.ToProtoGetFileStatRequest(args)  # pyrefly: ignore[bad-assignment]

        # pylint: disable=line-too-long
        # fmt: off
        if (args.pathspec.pathtype != jobs_pb2.PathSpec.PathType.REGISTRY or  # pyrefly: ignore[missing-attribute]
            args.pathspec.path != r"HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\SystemRoot"):  # pyrefly: ignore[missing-attribute]
        # pylint: enable=line-too-long
        # fmt: on
          raise OSError(f"Unsupported path: {args.pathspec}")  # pyrefly: ignore[missing-attribute]
      ...
      def ListDirectory(
        ...
        args = mig_client_action.ToProtoListDirRequest(args)  # pyrefly: ignore[bad-assignment]

        # fmt: off
        if (args.pathspec.pathtype != jobs_pb2.PathSpec.PathType.OS or  # pyrefly: ignore[missing-attribute]
            args.pathspec.path != "X:\\Users"):  # pyrefly: ignore[missing-attribute]
        # fmt: on
          raise OSError(f"Unsupported path: {args.pathspec}")  # pyrefly: ignore[missing-attribute]
  ...
  def testWindowsListUsersDirWithForwardAndLeadingSlashes(self):
    ...
    class ActionMock(action_mocks.ActionMock):
      ...
      def ListDirectory(
        ...
        if (
            args.pathspec.pathtype != jobs_pb2.PathSpec.PathType.OS  # pyrefly: ignore[missing-attribute]
            or args.pathspec.path != "X:\\Users"  # pyrefly: ignore[missing-attribute]
        ):
          raise OSError(f"Unsupported path: {args.pathspec}")  # pyrefly: ignore[missing-attribute]
  ...
  def testWindowsWmiUsersQuery(self):
    ...
    class ActionMock(action_mocks.ActionMock):
      ...
      def VfsFileFinder(
        ...
        if (
            args.action.action_type  # pyrefly: ignore[missing-attribute]
            != rdf_file_finder.FileFinderAction.Action.STAT  # pyrefly: ignore[missing-attribute]
            or args.pathtype != rdf_paths.PathSpec.PathType.REGISTRY  # pyrefly: ignore[missing-attribute]
        ):
          raise OSError("Unsupported arguments")

        if args.paths[0].startswith("HKEY_USERS"):  # pyrefly: ignore[missing-attribute]
