...
class FlowBase(
  ...
  proto_args_type: Type[_ProtoArgsT] = flows_pb2.EmptyFlowArgs  # pyrefly: ignore[bad-assignment]
  ...
  # message defined.
  proto_store_type: Type[_ProtoStoreT] = flows_pb2.DefaultFlowStore  # pyrefly: ignore[bad-assignment]
  ...
  # return type.
  proto_progress_type: Type[_ProtoProgressT] = flows_pb2.DefaultFlowProgress  # pyrefly: ignore[bad-assignment]
  ...
  def CallStateProto(
    ...
    if request_data is not None:
      flow_request.request_data.CopyFrom(models_protodicts.Dict(request_data))  # pyrefly: ignore[bad-argument-type]
  ...
  def _GetAndCheckResourceLimits(self) -> _ResourceLimits:
    ...
    return self._ResourceLimits(
        cpu_limit_ms=cpu_limit_ms,  # pyrefly: ignore[bad-argument-type]
  ...
  def CallClientProto(
    ...
    """
    try:
      action_registry.ID_BY_ACTION_STUB[action_cls]  # pyrefly: ignore[bad-index]
    except KeyError:
    ...
    if action_args:
      ...
      models_clients.SetGrrMessagePayload(
          client_action_request, action_cls.in_rdfvalue.__name__, action_args  # pyrefly: ignore[missing-attribute]
      )
  ...
  def CallFlowProto(
    ...
    """
    if not getattr(self, next_state):  # pyrefly: ignore[bad-argument-type]
      raise ValueError("Next state %s is invalid." % next_state)
  ...
  def SaveResourceUsage(self, status: rdf_flow_objects.FlowStatus) -> None:
    ...
    user_cpu = status.cpu_time_used.user_cpu_time  # pyrefly: ignore[missing-attribute]
    system_cpu = status.cpu_time_used.system_cpu_time  # pyrefly: ignore[missing-attribute]
    ...
    self.network_bytes_sent = (
        self.network_bytes_sent + status.network_bytes_sent  # pyrefly: ignore[missing-attribute]
    )

    if not self.runtime_us:
      self.runtime_us = 0

    if status.runtime_us:  # pyrefly: ignore[missing-attribute]
      runtime_us_int = status.runtime_us.SerializeToWireFormat()  # pyrefly: ignore[missing-attribute]
  ...
  def RunStateMethod(
    ...
    Args:
      ...
      responses: A list of FlowResponses and FlowStatuses responding to the
        request.
    ...
    if request and responses:
      logging.debug(
          ...
          client_id,
          self.flow_id,
          request.request_id,  # pyrefly: ignore[missing-attribute]
          len(responses),
      )
    else:
      logging.debug(
    ...
    try:
      ...
      if responses is not None and (
        ...
        responses = (
            flow_responses.Responses.FromResponsesProto2AnyWithOptionalStatus(  # pyrefly: ignore[bad-assignment]
                responses, request
            )
        )
        ...
        responses = flow_responses.Responses.FromResponsesProto2Any(  # pyrefly: ignore[bad-assignment]
            responses, request
        )
      else:
        responses = flow_responses.Responses.FromResponses(  # pyrefly: ignore[bad-assignment]
            request=request, responses=responses
        )

      if responses.status is not None:  # pyrefly: ignore[missing-attribute]
        self.SaveResourceUsage(responses.status)  # pyrefly: ignore[bad-argument-type]
  ...
  def ProcessAllReadyRequests(self) -> tuple[int, int]:
    ...
    for request, responses in incremental_requests:
      ...
      if rdf_responses:
        ...
        if flow_updates:
          self.RunStateMethod(request.callback_state, request, flow_updates)  # pyrefly: ignore[missing-attribute]

        # If the request was processed, update the next_response_id.
        next_response_id_map[request.request_id] = (  # pyrefly: ignore[missing-attribute]
  ...
  def _ProcessRepliesWithOutputPluginProto(
    ...
    for plugin_id, plugin_descriptor in existing_plugins:
      ...
      if plugin_cls.args_type is not None:  # pyrefly: ignore[missing-attribute]
        # `plugin_descriptor.args` is an instance of `any_pb2.Any`.
        pl_args = plugin_cls.args_type.FromString(plugin_descriptor.args.value)  # pyrefly: ignore[missing-attribute]
      else:
        pl_args = None
  ...
  @property
  def rrg_mode(self) -> flows_pb2.FlowRunnerArgs.RrgMode:
    """Returns whether RRG support is disabled for the flow."""
    return self.proto_flow.rrg_mode
  ...
  def rrg_version(self) -> RRGVersion:
    ...
    if self.rrg_mode == flows_pb2.FlowRunnerArgs.RrgMode.DISABLED:
  ...
  def GetDefaultArgs(cls, username: Optional[str] = None) -> _ProtoArgsT:
    del username  # Unused.
    return cls.proto_args_type()  # pyrefly: ignore[missing-attribute]
...
def UseProto2AnyResponses(
  ...
  Wrapper._proto2_any_responses = True  # pylint: disable=protected-access  # pyrefly: ignore[missing-attribute]
...
def UseProto2AnyResponsesCallback(
  ...
  Wrapper._proto2_any_responses_callback = True  # pylint: disable=protected-access  # pyrefly: ignore[missing-attribute]
...
def _TerminateFlow(
    ...
    flow_state: flows_pb2.Flow.FlowState = flows_pb2.Flow.FlowState.ERROR,
) -> None:
  ...
  proto_flow.error_message = reason  # pyrefly: ignore[bad-assignment]
  flow_obj.NotifyCreatorOfError()
  data_store.REL_DB.UpdateFlow(
      proto_flow.client_id,
      proto_flow.flow_id,
      flow_obj=proto_flow,
      processing_on=None,  # pyrefly: ignore[bad-argument-type]
