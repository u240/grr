...
class FlowBaseTest(
  ...
  def testClientInfo(self, db: abstract_db.Database):
    ...
    self.assertEqual(flow.client_info.client_name, "rrg")  # pyrefly: ignore[missing-attribute]
    self.assertEqual(flow.client_info.client_version, 1337)  # pyrefly: ignore[missing-attribute]
  ...
  def testClientInfoDefault(self, db: abstract_db.Database):
    ...
    flow = FlowBaseTest.Flow(flow)
    self.assertIsInstance(flow.client_info, rdf_client.ClientInformation)
    self.assertEmpty(flow.client_info.client_name)  # pyrefly: ignore[missing-attribute]
  ...
  def testRrgSupport_Disable(self, db: abstract_db.Database):
    ...
    flow.client_id = client_id
    flow.flow_id = self._FLOW_ID
    flow.rrg_mode = flows_pb2.FlowRunnerArgs.RrgMode.DISABLED
  ...
  def testResultMetadataHasGroupedNumberOfReplies(
    ...
    flow_obj = FlowBaseTest.Flow(proto_flow=flow)
    flow_obj.proto_result_types = (  # pyrefly: ignore[bad-assignment]
  ...
  def testResultMetadataAreCorrectlyUpdatedAfterMultiplePersistStateCalls(
    ...
    flow_obj = FlowBaseTest.Flow(proto_flow=flow)
    flow_obj.proto_result_types = (jobs_pb2.ClientInformation,)  # pyrefly: ignore[bad-assignment]
    flow_obj.SendReplyProto(jobs_pb2.ClientInformation())
    flow_obj.PersistState()
    flow_obj.PersistState()
    db.WriteFlowObject(flow_obj.proto_flow)
  ...
  def testSendReplyProtoRaisesIfNotProto(self, db: abstract_db.Database):
    ...
    with self.assertRaisesRegex(
      ...
      flow_obj.SendReplyProto(rdf_client.ClientInformation())  # pyrefly: ignore[bad-argument-type]
  ...
  def testRunStateMethodStaticAnyResponse(self):
    ...
    response.any_payload = rdf_structs.AnyValue.PackProto2(expected_response)  # pyrefly: ignore[missing-attribute]
  ...
  def testRunStateMethodStaticAnyResponseWithoutStatusShouldFail(
    ...
    response = rdf_flow_objects.FlowResponse()
    response.any_payload = rdf_structs.AnyValue.PackProto2(  # pyrefly: ignore[missing-attribute]
  ...
  def testRunStateMethodStaticAnyResponseCallback(self):
    ...
    class FakeStateCallbackMethodFlow(flow_base.FlowBase):
      ...
      @flow_base.UseProto2AnyResponsesCallback  # pyrefly: ignore[bad-argument-type]
      def FakeCallback(
          self,
          responses: flow_responses.Responses[any_pb2.Any],
      ) -> None:
  ...
  def testProcessAllReadyRequests_CallsRunStateMethodForCompletedRequests(
    ...
    flow_id = db_test_utils.InitializeFlow(
        ...
        flow_state=rdf_flow_objects.Flow.FlowState.RUNNING,  # pyrefly: ignore[missing-attribute]
    )
  ...
  def testCallStateInlineProtoWithResponses_AnyAndRequestData(self):
    class CallStateInlineResponsesAnyRequestData(flow_base.FlowBase):
      ...
      def ReceiveInlineResponses(self, responses):
        ...
        assert responses.request_data["hello"] == "world"  # pyrefly: ignore[unsupported-operation]
  ...
  def testCallStateProto_RequestData(self):
    ...
    class CallStateProtoWithRequestDataFlow(
      ...
      def AfterCallStateProto(
        ...
        assert responses.request_data["foo"] == "bar"  # pyrefly: ignore[unsupported-operation]
        assert responses.request_data["baz"] == 42  # pyrefly: ignore[unsupported-operation]
...
class FindFlowRequestsToProcessTest(absltest.TestCase):
  ...
  def testFindIncrementalRequestsToProcess(self):
    ...
    incremental_requests = flow_base.FindIncrementalRequestsToProcess(
        request_dict=flow_requests, next_needed_request_id=2  # pyrefly: ignore[bad-argument-type]
    )
  ...
  def testFindCompletedRequestsToProcess_ExcludeAfterIncompleteRequest(self):
    ...
    completed_requests = flow_base.FindCompletedRequestsToProcess(
        request_dict=flow_requests, next_needed_request_id=1  # pyrefly: ignore[bad-argument-type]
    )
