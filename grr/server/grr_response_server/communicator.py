...
class Communicator(metaclass=abc.ABCMeta):
  ...
  def EncodeMessageList(cls, message_list, packed_message_list):
    ...
    if len(compressed_data) < len(uncompressed_data):
      packed_message_list.compression = (
          rdf_flows.PackedMessageList.CompressionType.ZCOMPRESSION  # pyrefly: ignore[missing-attribute]
      )
  ...
  def DecompressMessageList(cls, packed_message_list):
    ...
    if compression == rdf_flows.PackedMessageList.CompressionType.UNCOMPRESSED:  # pyrefly: ignore[missing-attribute]
      data = packed_message_list.message_list

    elif (
        compression == rdf_flows.PackedMessageList.CompressionType.ZCOMPRESSION  # pyrefly: ignore[missing-attribute]
    ):
      try:
