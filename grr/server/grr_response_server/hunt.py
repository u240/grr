...
def _TryStopHunt(
    hunt_id: str,
    hunt_state_reason: hunts_pb2.Hunt.HuntStateReason.ValueType,
    reason_comment: str,
) -> None:
  """Stops the hunt unless somebody else stopped it in the meantime.

  Limits are enforced by workers processing client responses. Responses of many
  clients of the same hunt are processed concurrently, so multiple workers can
  observe the same limit being exceeded and race to stop the hunt. Only the
  first one wins and the losers should not fail because of that.

  Args:
    hunt_id: An identifier of the hunt to stop.
    hunt_state_reason: A reason for stopping the hunt.
    reason_comment: A human-readable description of the reason.
  """
  try:
    StopHunt(
        hunt_id,
...
def StopHuntIfCrashLimitExceeded(hunt_id: str) -> None:
  """Stops the hunt if number of crashes exceeds the limit."""
  hunt_obj = data_store.REL_DB.ReadHuntObject(hunt_id)

  # Do nothing if the hunt is not running or paused.
  ...
  if hunt_obj.crash_limit:
    ...
    if hunt_counters.num_crashed_clients >= hunt_obj.crash_limit:
      ...
      _TryStopHunt(hunt_obj.hunt_id, hunt_state_reason, reason)
...
def StopHuntIfCPUOrNetworkLimitsExceeded(hunt_id: str) -> None:
  ...
  if (
    ...
    _TryStopHunt(hunt_obj.hunt_id, hunt_state_reason, reason)
    # The hunt is stopped, no need to check the remaining limits.
    return
...
def CompleteHuntIfExpirationTimeReached(hunt_id: str) -> None:
