...
class FakeMemoryProcess(object):
  ...
  def __init__(self, pid=None, tmp_dir=None):
    self.pid = pid
    self.regions = self.regions_by_pid[pid]  # pyrefly: ignore[bad-index]
  ...
  def _WriteMemoryToFile(self) -> str:
    memory_path = os.path.join(self._tmp_dir, str(self.pid))  # pyrefly: ignore[no-matching-overload]
    prev_region = None
...
class BaseYaraFlowsTest(flow_test_lib.FlowTestsBaseclass):
  ...
  def process(self, processes, pid=None):
    for stack_frame in inspect.stack():
      ...
      if (
        ...
        return psutil.Process.old_target(pid=pid)  # pyrefly: ignore[missing-attribute]
    if not pid:
      return psutil.Process.old_target()  # pyrefly: ignore[missing-attribute]
    for p in processes:
      if p.pid == pid:
        return p
  ...
  def _RunYaraProcessScan(
      ...
      include_errors_in_results: rdf_memory.YaraProcessScanRequest.ErrorPolicy = "NO_ERRORS",  # pyrefly: ignore[missing-attribute]
  ...
  def testYaraSignatureReferenceDeliversFullSignatureToClient(self):
    ...
    class FakeYaraProcessScan(action_mocks.ActionMock):
      ...
      def YaraProcessScan(
        ...
        shards.append(args.signature_shard)  # pyrefly: ignore[missing-attribute]
        return []
...
def _FakeScanProcessMemoryYaraHandler(
  ...
  if filestore is not None:
    handlers |= rrg_test_lib.FakeFilestoreHandlers(filestore)  # pyrefly: ignore[unsupported-operation]
  return handlers
    ...
    with mock.patch.object(
      ...
      flow_id = rrg_test_lib.ExecuteFlow(
          ...
          handlers={
              ...
              **_FakeDumpProcessMemoryHandler(process_memory),  # pyrefly: ignore[bad-argument-type]
          },
      )
