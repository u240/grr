  ...
  def testLinuxDebianPost3478DpkgFallback(self) -> None:
    ...
    class ActionMock(action_mocks.ActionMock):
      ...
      def ExecuteCommand(
        ...
        args = mig_client_action.ToProtoExecuteRequest(args)  # pyrefly: ignore[bad-assignment]
        result = jobs_pb2.ExecuteResponse()

        if args.cmd == "/usr/bin/dpkg-query":  # pyrefly: ignore[missing-attribute]
...
""".encode("utf-8")
  ...
  def testLinuxDebian_RRG(self, db: abstract_db.Database) -> None:
    # TODO - Load signed commands from the `.textproto` file to
    # ensure integrity.
    dpkg_query_command = rrg_execute_signed_command_pb2.Command()
    dpkg_query_command.path.raw_bytes = "/usr/bin/dpkg-query".encode()
    db.WriteSignedCommand(
        signed_commands_pb2.SignedCommand(
            id="dpkg_query_show_showformat",
            operating_system=signed_commands_pb2.SignedCommand.LINUX,
            command=dpkg_query_command.SerializeToString(),
            ed25519_signature=b"\x00" * 64,
        )
    )

    rpm_query_command = rrg_execute_signed_command_pb2.Command()
    rpm_query_command.path.raw_bytes = "/bin/rpm".encode()
    db.WriteSignedCommand(
        signed_commands_pb2.SignedCommand(
            id="rpm_query_all_queryformat",
            operating_system=signed_commands_pb2.SignedCommand.LINUX,
            command=rpm_query_command.SerializeToString(),
            ed25519_signature=b"\x00" * 64,
        )
    )
    ...
    def ExecuteSignedCommandHandler(session: rrg_test_lib.Session) -> None:
      ...
      assert command.path.raw_bytes.decode() == "/usr/bin/dpkg-query"
...
""".encode("utf-8")
    ...
    flow_id = rrg_test_lib.ExecuteFlow(
        client_id=client_id,
        flow_cls=software.CollectInstalledSoftware,
...
gpg-pubkey|(none)|18b8e74c|62f2920f|(none)|1711525880|(none)|(none)
  ...
  @db_test_lib.WithDatabase
  def testLinuxFedora_RRG(self, db: abstract_db.Database) -> None:
    # TODO - Load signed commands from the `.textproto` file to
    # ensure integrity.
    dpkg_query_command = rrg_execute_signed_command_pb2.Command()
    dpkg_query_command.path.raw_bytes = "/usr/bin/dpkg-query".encode()
    ...
    def ExecuteSignedCommandHandler(session: rrg_test_lib.Session) -> None:
      ...
      assert command.path.raw_bytes.decode() == "/bin/rpm"
...
""".encode("utf-8")
    ...
    # TODO - Update version checks once we know what the proper way
    # forward for reporting it is.
  ...
  @db_test_lib.WithDatabase
  def testWindows_RRG(self, db: abstract_db.Database) -> None:
    client_id = db_test_utils.InitializeRRGClient(
        db,
  ...
  def testWindows(self) -> None:
    ...
    class ActionMock(action_mocks.ActionMock):
      ...
      def WmiQuery(
        ...
        args = mig_client_action.ToProtoWMIRequest(args)  # pyrefly: ignore[bad-assignment]

        if not args.query.upper().startswith("SELECT "):  # pyrefly: ignore[missing-attribute]
          raise RuntimeError("Non-`SELECT` WMI query")

        if "Win32_Product" in args.query:  # pyrefly: ignore[missing-attribute]
          for result in [
              {
                  "Name": "Rust 1.76 (MSVC 64-bit)",
                  "InstallDate": "20231229",
            ...
            yield mig_protodict.ToRDFDict(models_protodicts.Dict(result))  # pyrefly: ignore[bad-argument-type]
        elif "Win32_QuickFixEngineering" in args.query:  # pyrefly: ignore[not-iterable]
          for result in [
              {
                  "HotFixID": "KB5033909",
                  "InstalledOn": "1/10/2024",
  ...
  @db_test_lib.WithDatabase
  def testMacos_RRG(self, db: abstract_db.Database) -> None:
    client_id = db_test_utils.InitializeRRGClient(
        db,
  ...
  def testMacos(self) -> None:
    ...
    class FakeFileFinderOS(actions.ActionPlugin):
      ...
      def Run(self, args: rdf_file_finder.FileFinderArgs) -> None:
        args = mig_file_finder.ToProtoFileFinderArgs(args)  # pyrefly: ignore[bad-assignment]

        if args.pathtype != jobs_pb2.PathSpec.PathType.OS:  # pyrefly: ignore[missing-attribute]
          raise RuntimeError(f"Unexpected path type: {args.pathtype}")

        if list(args.paths) != ["/Library/Receipts/InstallHistory.plist"]:  # pyrefly: ignore[missing-attribute]
          raise RuntimeError(f"Unexpected paths: {args.paths}")  # pyrefly: ignore[missing-attribute]
