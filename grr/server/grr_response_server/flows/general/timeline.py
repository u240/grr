...
class TimelineFlow(
  ...
  def Start(self) -> None:
    ...
    if not self.client_info.timeline_btime_support:  # pyrefly: ignore[missing-attribute]
    ...
    if self.rrg_support:
      ...
      action.Call(self.HandleRRGGetFilesystemTimeline)  # pyrefly: ignore[bad-argument-type]
    else:
