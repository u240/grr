...
class OsqueryFlow(
  ...
  def Start(self):
    ...
    if self.rrg_support and not self.proto_args.configuration_content:
      ...
      action.Call(self._ProcessRRG)  # pyrefly: ignore[bad-argument-type]
    elif (
      ...
      action.Call(self._ProcessStoreConfig)  # pyrefly: ignore[bad-argument-type]
    else:
  ...
  def Process(
    ...
    if not responses.success:
      ...
      message = f"{status.error_message}: {status.backtrace}"  # pyrefly: ignore[missing-attribute]
      self._UpdateProgressWithError(message)
