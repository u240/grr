...
class ListDirectory(
  ...
  def Start(self):
    ...
    if (
        self.rrg_support
        and self.proto_args.pathspec.pathtype == rdf_paths.PathSpec.PathType.OS  # pyrefly: ignore[missing-attribute]
    ):
      path_raw_bytes = self.proto_args.pathspec.path.encode("utf-8")
      ...
      action.Call(self.HandleRRGGetFileMetadata)  # pyrefly: ignore[bad-argument-type]
    else:
  ...
  def HandleRRGGetFileMetadata(
    ...
    if response_dir.symlink.raw_bytes:
      ...
      get_file_metadata.Call(self.HandleRRGGetFileMetadata)  # pyrefly: ignore[bad-argument-type]
  ...
  def NotifyAboutEnd(self):
    ...
    ps_path_type = st.pathspec.last.pathtype  # pyrefly: ignore[missing-attribute]
    path_type = rdf_objects.PathInfo.PathTypeFromPathspecPathType(ps_path_type)

    full_path = st.pathspec.CollapsePath()  # pyrefly: ignore[missing-attribute]
...
class RecursiveListDirectory(
  ...
  def Start(self):
    ...
    if (
        self.rrg_support
        and self.proto_args.pathspec.pathtype == jobs_pb2.PathSpec.OS
    ):
      path_raw_bytes = self.proto_args.pathspec.path.encode("utf-8")
      # TODO - Sometimes GRR "fixes" Windows paths and inserts a
      # leading '/' in front (e.g. to have `/C:/Windows`). RRG does not treat it
      ...
      action = rrg_stubs.GetFileMetadata()
      action.args.paths.add().raw_bytes = path_raw_bytes
      action.args.max_depth = self.proto_args.max_depth
  ...
  def _ProcessGetFileMetadata(
    ...
    if not responses_any.success:
      raise flow_base.FlowError(
          f"Failed to collect metadata: {responses_any.status}",
      )
    ...
    for response_any in responses_any:
      ...
      # Note that unlike the non-recursive directory listing flow, here we do
      # not want to follow any symlinks.
      if response.symlink.raw_bytes:
        symlink = rrg_path.PurePath.For(self.rrg_os_type, response.symlink)
  ...
  def ProcessDirectory(
    ...
    directory_pathspec = first_rdf.pathspec.Dirname()  # pyrefly: ignore[missing-attribute]
