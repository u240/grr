...
class TestFileFinderFlow(
  ...
  def testDownloadAndHashActionSizeLimitWithHashTruncatedPolicy(self):
    ...
    for action in [hash_action, download_action]:
      ...
      with self.assertRaises(file_store.FileHasNoContentError):
        self._ReadTestFile(
            ["test_img.dd"], path_type=rdf_objects.PathInfo.PathType.OS  # pyrefly: ignore[missing-attribute]
        )
  ...
  def testDownloadActionWithMultipleAttemptsWithMultipleSizeLimits(self):
    ...
    pathspec = rdf_paths.PathSpec(
        pathtype=rdf_paths.PathSpec.PathType.OS, path=path  # pyrefly: ignore[missing-attribute]
    )
  ...
  def _RunTSKFileFinder(self, paths):
    ...
    with mock.patch.object(
        ...
        "_VFS_VIRTUALROOTS",
        {
            rdf_paths.PathSpec.PathType.TSK: rdf_paths.PathSpec(  # pyrefly: ignore[missing-attribute]
...
class TestClientFileFinderFlow(flow_test_lib.FlowTestsBaseclass):
  ...
  @db_test_lib.WithDatabase
  def testRRG_Stat_Overlapping(self, rel_db: db.Database):
    client_id = db_test_utils.InitializeRRGClient(
        rel_db,
        os_type=rrg_os_pb2.LINUX,
    )
    ...
    flow_id = rrg_test_lib.ExecuteFlow(
        ...
        handlers=rrg_test_lib.FakePosixFileHandlers({
            "/foo/bar": b"Lorem ipsum.",
            "/foo/baz": b"Dolor sit amet.",
            "/foo/bax": b"Consectetur adipiscing elit.",
        }),
    )
    ...
    for flow_result in flow_results:
      ...
      self.assertNotIn(result.stat_entry.pathspec.path, results_by_path)
      results_by_path[result.stat_entry.pathspec.path] = result

    self.assertEqual(  # pylint: disable=g-generic-assert
        results_by_path["/foo/bar"].stat_entry.st_size,
        len(b"Lorem ipsum."),
    )
    self.assertEqual(  # pylint: disable=g-generic-assert
        results_by_path["/foo/baz"].stat_entry.st_size,
        len(b"Dolor sit amet."),
    )
    self.assertEqual(  # pylint: disable=g-generic-assert
        results_by_path["/foo/bax"].stat_entry.st_size,
  ...
  @db_test_lib.WithDatabase
  def testRRG_Download_NTFS(self, rel_db: db.Database):
    client_id = db_test_utils.InitializeRRGClient(
        rel_db,
        os_type=rrg_os_pb2.WINDOWS,
    )
    ...
    args = flows_pb2.FileFinderArgs()
    args.action.action_type = flows_pb2.FileFinderAction.Action.DOWNLOAD
    args.pathtype = jobs_pb2.PathSpec.PathType.NTFS
    ...
    flow_id = rrg_test_lib.ExecuteFlow(
        ...
        rrg_mode=flows_pb2.FlowRunnerArgs.RrgMode.FORCED,
    )
  ...
  def testRRG_Empty(self, rel_db: db.Database):
    ...
    self.assertEqual(result.stat_entry.st_size, 0)
    self.assertEqual(result.hash_entry.md5, hashlib.md5(b"").digest())
    self.assertEqual(result.hash_entry.sha1, hashlib.sha1(b"").digest())
    self.assertEqual(result.hash_entry.sha256, hashlib.sha256(b"").digest())
    ...
    self.assertEqual(path_info.stat_entry.st_size, 0)
    self.assertEqual(path_info.hash_entry.sha256, hashlib.sha256(b"").digest())
  ...
  @db_test_lib.WithDatabase
  def testRRG_EmptyAndNotEmpty(self, rel_db: db.Database):
    client_id = db_test_utils.InitializeRRGClient(
        rel_db,
        os_type=rrg_os_pb2.LINUX,
    )
    ...
    self.assertEqual(
        results_by_path["/empty"].stat_entry.st_size,
        0,
    )
    self.assertEqual(
        results_by_path["/empty"].hash_entry.md5,
        hashlib.md5(b"").digest(),
    )
    self.assertEqual(
        results_by_path["/empty"].hash_entry.sha1,
        hashlib.sha1(b"").digest(),
    )
    self.assertEqual(
        results_by_path["/empty"].hash_entry.sha256,
        hashlib.sha256(b"").digest(),
    )

    self.assertEqual(  # pylint: disable=g-generic-assert
        results_by_path["/nonempty"].stat_entry.st_size,
        len(b"Lorem ipsum."),
    )
    self.assertEqual(
        results_by_path["/nonempty"].hash_entry.md5,
        hashlib.md5(b"Lorem ipsum.").digest(),
    )
    self.assertEqual(
        results_by_path["/nonempty"].hash_entry.sha1,
        hashlib.sha1(b"Lorem ipsum.").digest(),
    )
    self.assertEqual(
        results_by_path["/nonempty"].hash_entry.sha256,
        hashlib.sha256(b"Lorem ipsum.").digest(),
    )

    path_info_empty = rel_db.ReadPathInfo(
        client_id=client_id,
        path_type=objects_pb2.PathInfo.PathType.OS,
        components=["empty"],
    )
    ...
    path_info_nonempty = rel_db.ReadPathInfo(
        client_id=client_id,
        path_type=objects_pb2.PathInfo.PathType.OS,
        components=["nonempty"],
    )
    self.assertEqual(  # pylint: disable=g-generic-assert
        path_info_nonempty.stat_entry.st_size,
        len(b"Lorem ipsum."),
    )
    self.assertEqual(
        path_info_nonempty.hash_entry.sha256,
        hashlib.sha256(b"Lorem ipsum.").digest(),
    )
    ...
    file_nonempty = file_store.OpenFile(
        db.ClientPath.OS(
            client_id=client_id,
            components=["nonempty"],
        )
    )
    self.assertEqual(file_nonempty.read(), b"Lorem ipsum.")
