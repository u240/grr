...
def OpenFile(
  ...
  hash_id = rdf_objects.SHA256HashID.FromSerializedBytes(
      path_info.hash_entry.sha256.AsBytes()  # pyrefly: ignore[missing-attribute]
  )
  ...
  return BlobStream(client_path, blob_references, hash_id)  # pyrefly: ignore[bad-argument-type]
...
def StreamFilesChunks(
  ...
  for cp in client_paths:
    ...
    num_blobs = len(blob_refs)  # pyrefly: ignore[bad-argument-type]
    total_size = 0
    for ref in blob_refs:  # pyrefly: ignore[not-iterable]
      total_size += ref.size

    cur_size = 0
    for i, ref in enumerate(blob_refs):  # pyrefly: ignore[bad-argument-type, not-iterable]
