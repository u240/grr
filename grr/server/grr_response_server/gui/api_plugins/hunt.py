...
def InitApiHuntFromHuntObject(
  ...
  if hunt_obj.args.hunt_type == hunts_pb2.HuntArguments.HuntType.STANDARD:
    ...
    api_hunt.hunt_type = hunts_pb2.HuntArguments.HuntType.STANDARD  # pyrefly: ignore[bad-assignment]
  else:
    api_hunt.name = "VariableGenericHunt"
    api_hunt.hunt_type = hunts_pb2.HuntArguments.HuntType.VARIABLE  # pyrefly: ignore[bad-assignment]
...
class ApiListHuntsHandler(api_call_handler_base.ApiCallHandler):
  ...
  def Handle(
    ...
    kw_args = {}
    if args.created_by:
      kw_args["with_creator"] = self._Username(args.created_by, context)  # pyrefly: ignore[bad-argument-type]
...
class ApiListHuntOutputPluginLogsHandlerBase(
  ...
  def Handle(
    ...
    return self.proto_result_type(  # pyrefly: ignore[not-callable]
        total_count=total_count,
        items=[
...
class ApiGetHuntContextHandler(api_call_handler_base.ApiCallHandler):
  ...
  def Handle(
    ...
    context = flows_pb2.HuntContext(  # pyrefly: ignore[bad-assignment]
    ...
    return api_hunt_pb2.ApiGetHuntContextResult(
        context=context, state=api_utils_pb2.ApiDataObject()  # pyrefly: ignore[bad-argument-type]
    )
