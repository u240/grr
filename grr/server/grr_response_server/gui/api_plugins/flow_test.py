...
class ApiCreateFlowTest(absltest.TestCase):
  ...
  @db_test_lib.WithDatabase
  def testRRGModeDisabled(self, db: abstract_db.Database):
    ...
    args.flow.runner_args.rrg_mode = flows_pb2.FlowRunnerArgs.RrgMode.DISABLED

    result = handler.Handle(args, context=_CreateContext(db))
    self.assertEqual(
        result.runner_args.rrg_mode,
