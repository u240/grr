...
/**
 * Whether a Fleet Collection can be (re)started.
 *
 * A Fleet Collection is paused server-side both before it is started for the
 * first time (NOT_STARTED) and after it hits its client limit
 * (REACHED_CLIENT_LIMIT), so it can be (re)started from either state.
 */
export function canStartFleetCollection(
  fleetCollection: Hunt | null | undefined,
): boolean {
  return (
    fleetCollection?.state === HuntState.NOT_STARTED ||
    fleetCollection?.state === HuntState.REACHED_CLIENT_LIMIT
  );
}
...
export const FleetCollectionStore = signalStore(
  ...
  withComputed((store) => ({
    ...
    canStartFleetCollection: computed(() =>
      canStartFleetCollection(store.fleetCollection()),
    ),
