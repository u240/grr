...
class FleetCollectionTest(gui_test_lib.GRRSeleniumHuntTest):
  ...
  def testRestartPausedFleetCollection(self):
    client_id = self.SetupClient(0)
    fleet_collection_id = self.StartHunt(
        flow_runner_args=flows_pb2.FlowRunnerArgs(
            flow_name=discovery.Interrogate.__name__
        ),
        flow_args=flows_pb2.InterrogateArgs(),
    ...
    # There is no foreman running in the tests, so the Fleet Collection has to
    # be assigned to the client explicitly. Running it on a single client
    # reaches its client limit of 1, which makes the server pause it.
    self.AssignTasksToClients([client_id])

    self.Open(f"/fleet-collections/{fleet_collection_id}/configuration")
    self.WaitUntil(self.IsTextPresent, "Paused - reached client limit")
